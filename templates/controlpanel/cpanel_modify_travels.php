...
//            session_start();
        ...
        if ($view == 'edit') {
            ...
            if (!$query) {
                die("connection failed" . mysql_error());
                header("location: /templates/controlpanel/cpanel_dashboard.php?view=add_travels&purpose=add&message=failure");
            } else {
                header("location: /templates/controlpanel/cpanel_dashboard.php?view=add_travels&purpose=add&message=success");
            }
        } else {
            echo '<form style="margin-bottom: 10px;" name="
                    editTsForm" id="editTsForm" method="get" action="./cpanel_dashboard.php"  >
