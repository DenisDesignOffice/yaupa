...
//            session_start();
        ...
        if ($view == 'delete') {
            ...
            $query = "DELETE FROM taxi_bookings where id='" . $id . "'";
            
                if ($_POST['confirm'] !== 1) {
                mysql_query($query, $connect);
                }

            header("location: /templates/controlpanel/cpanel_dashboard.php?view=taxi_bookings");
        }else {
            
            $id =  strtolower(htmlspecialchars($_GET{'id'}));
            
            $query = 'SELECT id,status FROM taxi_bookings where id="' . $id . '"';
            ...
            $action = "UPDATE taxi_bookings SET status='$status' "
                    . "WHERE id=$id";
